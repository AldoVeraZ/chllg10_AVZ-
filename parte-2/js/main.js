...
function Libro(titulo, autor, precio, isbn, paginas, idioma, foto, sinopsis) {
  ...
  // Método para mostrar datos en ventana de alerta
  ...
  // Método para obtener el HTML de un artículo
  this.getHtmlArticle = function () {
    // Variable local para almacenar el título modificado
    let modifiedTitle = this.titulo;

    // Verificar si el título no comienza con "::"
    if (!this.titulo.startsWith("::")) {
      // Si no comienza con "::", añadir el prefijo a la variable local
      modifiedTitle = `:: ${this.titulo}`;
    }

    // Construir y retornar el HTML del artículo
    return `
      <article>
        <h2> ${this.titulo}</h2>  
...
// Crea el nuevo objeto Libro con la información de prueba
...
// Llamar al método mostrarDatosEnConsola para visualizar la información en la consola
libro1.mostrarDatos("console");

libro1.mostrarDatosEnConsolaPorParametro();

// Llamar al método getHtmlArticle para obtener el HTML del artículo
let htmlArticle = libro1.getHtmlArticle();
console.log(htmlArticle); // Imprimir el HTML en la consola

/* IMPORTANTE */
// Obtener el elemento <section> del DOM
let sectionLibros = document.getElementById("libros");

// Crear un nuevo elemento <article>
let nuevoArticle = document.createElement("article");

// Asignar el contenido generado por getHtmlArticle al nuevo <article>
nuevoArticle.innerHTML = libro1.getHtmlArticle();

// Agregar el nuevo <article> como hijo del <section>
sectionLibros.appendChild(nuevoArticle);

// Verificar en la consola el nuevo contenido
console.log(sectionLibros.innerHTML);

// Crear instancias de los objetos Libro
let libroUno = new Libro(
  "Sol de medianoche",
  "Stephenie Meyer",
  "$4.399,00",
  "9789877387308",
...
elemLibrosContainer.appendChild(nuevoArticle);

// Obtener el HTML de cada libro
let htmlLibroUno = libroUno.getHtmlArticle();
let htmlLibroDos = libroDos.getHtmlArticle();
let htmlLibroTres = libroTres.getHtmlArticle();
let htmlLibroCuatro = libroCuatro.getHtmlArticle();

// Concatenar el HTML de todos los libros
let htmlNuevosLibros =
  htmlLibroUno + htmlLibroDos + htmlLibroTres + htmlLibroCuatro;

// Agregar el HTML de los nuevos libros al contenedor
